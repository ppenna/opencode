...
First download or build NVX. The `path` can point to either an extracted NVX release (containing `bin/openvmm` and `guest/`) or an NVX checkout after running `python3 scripts/nvx.py download`.
